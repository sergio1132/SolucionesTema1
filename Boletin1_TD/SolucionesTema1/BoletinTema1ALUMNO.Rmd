...
Asignamos a una variable los dos conjuntos:
...
Utilizamos las funciones dim, head, tail, str, para una primera visión de los conjuntos de datos.
...
```{r}
diferentesM <- setdiff(rownames(dfm), rownames(dfa))
diferentesM
```

Son 4. Son **"Arctic fox" ,"Arctic ground squirrel", "Patas monkey" y "Mole rat"**.
...
diferentesA <- setdiff(rownames(dfa), rownames(dfm))
diferentesA
```

Son 7. Son **"Dipliodocus" ,"Potar monkey","Triceratops",Brachiosaurus","Mole" ,"Artic fox","Artic ground squirrel".**

e.  Determina las diferencia entre los animales que no aparecen en ambos conjuntos.

Los animales son los mismos, pero tienen nombres diferentes.
...
4.  \#Presentan un compartimiento bastante parecido.

    \#Sí porque las muestras se encuentran dentro de las las líneas discontinuas.La función **library** sin argumentos abre una ventana y muestra las librerías que han sido instaladas.
    ...
    #C):
    length(libReturn)
    #Tengo 3 librerias instaladas.
    ```

5.  En las transparencias del tema 1 se citan los primeros pasos a seguir cuando se analiza un nuevo conjunto de datos.
...
Sí que podemos asumir esta variable como una distribuciób Gaussiana porque sus muestras se encuentran dentro de las las líneas discontinuas.
...
```{r}
hist(cabbages$VitC[cabbages$Cult=="c39"],col = "lightcyan")
hist(cabbages$VitC[cabbages$Cult=="c52"],col = "lightgreen")
```

Sí que hay difrencias entre los contenidos de las dos variables, que se observa en los histogramas.
...
Primero comparamos las dos variables que nos piden:

```{r}
plot(cabbages$VitC ~ cabbages$HeadWt)
```

Observamos que no existe ninguna relación lineal para estas variables.

B):

Comparamos **VitC** en función de **HeadWt**.

```{r}
plot(cabbages$VitC ~ cabbages$HeadWt, pch = c(1,18)[cabbages$Cult])

Model39 <- lm(cabbages$VitC ~ cabbages$HeadWt, data = cabbages, subset = which(Cult == "c39"))
Model52 <- lm(cabbages$VitC ~ cabbages$HeadWt, data = cabbages, subset = which(Cult == "c52"))

abline(Model39, lty=4, col = "green")
abline(Model52, col = "blue")
```
